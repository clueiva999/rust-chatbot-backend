//! Fully‑async OAuth‑aware, token‑streaming WebSocket backend
//! for HireMage AI Assistant (Groq API) with Image Support.

mod transcript;
mod transcript_service;
mod s3_service;
mod db_service;
mod transcript_websocket;
mod transcript_api;
mod error_handler;
...
type OAClient = Client<OpenAIConfig>; // convenient alias

// ─── Constants
const MODEL_ID: &str = "openai/gpt-oss-120b";
const HOUSE_RULES_PROMPT: &str = r#"<core_identity>
You are Clueiva, developed and created by Clueiva, and you are the user's live-meeting co-pilot.
</core_identity>
...
- **"assistant"**: You (Clueiva) - SEPARATE from the above two
</speaker_label_understanding>
...
- If asked what model is running or powering you or who you are, respond: "I am Clueiva powered by a collection of LLM providers". NEVER mention the specific LLM providers or say that Clueiva is the AI itself.
...
enum WsMsg {
    ...
    #[serde(rename = "chat")]
    Chat { message: String },
    #[serde(rename = "image_chat")]
    ImageChat { message: String, image_data: String },

    // server → client
    #[serde(rename = "auth_response")]
    AuthResponse { success: bool, error: Option<String>, user: Option<AuthenticatedUser> },
    #[serde(rename = "system_message")]
    SystemMessage { message: String },
    #[serde(rename = "chat_delta")]
    ChatDelta { delta: String, done: bool },
    #[serde(rename = "error")]
...
pub struct AppState {
    pub openai: OAClient,
    pub authenticated: Arc<RwLock<HashMap<String, SessionContext>>>,
    pub transcript_service: Arc<transcript_service::TranscriptService>,
    pub integrated_service: Arc<transcript_integration::IntegratedTranscriptService>,
}
type Shared = Arc<AppState>;

// ─── Static index
async fn serve_index() -> impl IntoResponse {
    Html("<h1>HireMage AI backend OK</h1>")
}
...
// ─── WS main loop
async fn ws_loop(mut socket: WebSocket, state: Shared) {
    let sid = Uuid::new_v4().to_string();
    let mut is_authed = false;
    info!("WS connected {sid}");

    while let Some(Ok(Message::Text(txt))) = socket.recv().await {
        match serde_json::from_str::<WsMsg>(&txt) {
            Ok(WsMsg::Authenticate { token }) => {
                match authenticate_user(&token).await {
                    Ok(sess) => {
                        state.authenticated.write().await.insert(sid.clone(), sess.clone());
                        is_authed = true;

                        let ok = WsMsg::AuthResponse { success: true, error: None, user: Some(sess.user.clone()) };
                        socket.send(Message::Text(serde_json::to_string(&ok).unwrap())).await.ok();

                        let name = sess.user.name.as_deref().unwrap_or(&sess.user.email);
                        let welcome = WsMsg::SystemMessage { message: format!("# 👋 Welcome, {name}! 🟦 **Pro streaming ready**") };
                        socket.send(Message::Text(serde_json::to_string(&welcome).unwrap())).await.ok();
                    }
                    Err(e) => {
                        socket.send(Message::Text(serde_json::to_string(&WsMsg::AuthResponse {
                            success: false, error: Some(e), user: None,
                        }).unwrap())).await.ok();
                    }
                }
            }

            Ok(WsMsg::UpdateContext { context }) => {
                if !is_authed {
                    let err = WsMsg::Error { message: "Authentication required before updating context.".into() };
                    socket.send(Message::Text(serde_json::to_string(&err).unwrap())).await.ok();
                    continue;
                }
                let mut sessions = state.authenticated.write().await;
                if let Some(sess) = sessions.get_mut(&sid) {
                    sess.ai_context = context;
                    let ok = WsMsg::SystemMessage { message: "✅ Context updated.".into() };
                    socket.send(Message::Text(serde_json::to_string(&ok).unwrap())).await.ok();
                }
            }

            Ok(WsMsg::Chat { message }) => {
                if guard_pro(&mut socket, &state, &sid, is_authed).await.is_err() {
                    continue;
                }
                if let Err(e) = handle_chat(&mut socket, &state, &sid, message).await {
                    error!("{e:?}");
                    let _ = socket.send(Message::Text(serde_json::to_string(&WsMsg::Error { message: format!("❌ {e}") }).unwrap())).await;
                }
            }

            Ok(WsMsg::ImageChat { message, image_data }) => {
                if guard_pro(&mut socket, &state, &sid, is_authed).await.is_err() {
                    continue;
                }
                if let Err(e) = handle_image_chat(&mut socket, &state, &sid, message, image_data).await {
                    error!("{e:?}");
                    let _ = socket.send(Message::Text(serde_json::to_string(&WsMsg::Error { message: format!("❌ {e}") }).unwrap())).await;
                }
            }
    ...
    state.authenticated.write().await.remove(&sid);
    info!("WS disconnected {sid}");
}

// ─── streaming chat
async fn handle_chat(socket: &mut WebSocket, state: &Shared, sid: &str, user_txt: String) -> anyhow::Result<()> {
    const MAX_TURNS: usize = 10;

    // pull session
    let (ai_ctx, hist) = {
        let s = state.authenticated.read().await;
        let sref = s.get(sid).expect("session exists");
        (sref.ai_context.clone(), sref.chat_history.clone())
    };

    // build msgs
    let mut msgs = vec![ChatCompletionRequestMessage::System(
        ChatCompletionRequestSystemMessageArgs::default()
            .content(format!("{HOUSE_RULES_PROMPT}\n\n{ai_ctx}"))
            .build()?
    )];
    for m in hist.iter().rev().take(MAX_TURNS).rev() { msgs.push(m.clone()); }

    let user_msg = ChatCompletionRequestMessage::User(
        ChatCompletionRequestUserMessageArgs::default().content(user_txt.clone()).build()?
    );
    msgs.push(user_msg.clone());

    // store user turn (with proper lock lifetime)
    {
        let mut sessions = state.authenticated.write().await;
        if let Some(sess) = sessions.get_mut(sid) {
            sess.chat_history.push(user_msg);
        }
    }
    ...
    let mut stream = state.openai.chat().create_stream(req).await?;
    let mut assistant_full = String::new();

    while let Some(chunk) = stream.next().await {
        let CreateChatCompletionStreamResponse { choices, .. } = chunk?;
        if let Some(content) = choices.first().and_then(|c| c.delta.content.clone()) {
            assistant_full.push_str(&content);
            let delta = WsMsg::ChatDelta { delta: content, done: false };
            socket.send(Message::Text(serde_json::to_string(&delta)?)).await?;
        }
    }

    socket.send(Message::Text(serde_json::to_string(&WsMsg::ChatDelta { delta: String::new(), done: true })?)).await?;

    // persist assistant turn (proper lock lifetime)
    let assistant_msg = ChatCompletionRequestMessage::Assistant(
        ChatCompletionRequestAssistantMessageArgs::default().content(assistant_full.clone()).build()?
    );
    {
        let mut sessions = state.authenticated.write().await;
        if let Some(sess) = sessions.get_mut(sid) {
            sess.chat_history.push(assistant_msg);
            if sess.chat_history.len() > MAX_TURNS * 2 {
                sess.chat_history
                    .drain(0..sess.chat_history.len() - MAX_TURNS * 2);
            }
        }
    }
...
// ─── streaming image chat using async-openai vision API
async fn handle_image_chat(socket: &mut WebSocket, state: &Shared, sid: &str, user_txt: String, image_data: String) -> anyhow::Result<()> {
    const MAX_TURNS: usize = 10;
    ...
    // Create user message with image content using proper async-openai vision format
    let user_msg = ChatCompletionRequestMessage::User(
    ...
    // request using maverick model (supports both text and vision)
    let req = CreateChatCompletionRequestArgs::default()
        .model(MODEL_ID)
        .messages(msgs.clone())
        .temperature(0.2)
...
// ─── guards
async fn guard_pro(socket: &mut WebSocket, state: &Shared, sid: &str, authed: bool) -> Result<(), ()> {
    if !authed {
        socket.send(Message::Text(serde_json::to_string(&WsMsg::Error { message: "🔄 Authenticating…".into() }).unwrap())).await.ok();
        return Err(());
    }
    if !state.authenticated.read().await.get(sid).map(|s| s.user.is_pro).unwrap_or(false) {
        socket.send(Message::Text(serde_json::to_string(&WsMsg::Error { message: "🔒 Pro subscription required.".into() }).unwrap())).await.ok();
        return Err(());
    }
    Ok(())
}
...
async fn authenticate_user(tok: &str) -> Result<SessionContext, String> {
    let info = fetch_user_context(tok).await.map_err(|e| e.to_string())?;

    let u = info.get("user").ok_or("bad user")?;
    let user = AuthenticatedUser {
        id:    u.get("id").and_then(|v| v.as_str()).unwrap_or_default().into(),
        email: u.get("email").and_then(|v| v.as_str()).unwrap_or_default().into(),
        name:  u.get("name").and_then(|v| v.as_str()).map(|s| s.into()),
        is_pro:u.get("isPro").and_then(|v| v.as_bool()).unwrap_or(false),
    };

    // Allow authentication for all users, but handle AI context based on Pro status
    let ai_context = if user.is_pro {
        // Pro users need AI context for chat functionality
        info.get("aiContext")
            .and_then(|v| v.as_str())
            .filter(|s| !s.trim().is_empty())
            .ok_or("AI context missing - please set up your AI context first")?
            .to_string()
    } else {
        // Non-Pro users get a default message indicating upgrade needed
        "Upgrade to Pro subscription required for AI assistant access".to_string()
    };

    Ok(SessionContext {
        user, ai_context, chat_history: Vec::new(),
        access_token: tok.into(), authenticated_at: Utc::now().timestamp(),
    })
}
...
async fn fetch_user_context(tok: &str) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    if !r.status().is_success() { return Err(format!("ctx http {}", r.status()).into()) }
    Ok(r.json().await?)
}
...
async fn main() -> anyhow::Result<()> {
    // Load .env first so RUST_LOG and other variables are available to the logger
    dotenv().ok();

    // Setup logging: use RUST_LOG if present, otherwise default to "info"
    ...
    let openai_cfg = OpenAIConfig::new().with_api_key(api_key).with_api_base("https://api.groq.com/openai/v1");
    let openai = Client::with_config(openai_cfg);

    // Initialize transcript services
    let transcript_config = transcript_service::TranscriptServiceConfig::default();
    let transcript_service = Arc::new(transcript_service::TranscriptService::new(transcript_config));

    // Initialize S3 service
    let s3_config = s3_service::S3ServiceConfig::default();
    let s3_service = Arc::new(s3_service::S3Service::new(s3_config).await?);

    // Initialize database service
    let db_config = db_service::DbServiceConfig::default();
    let db_service = Arc::new(db_service::DbService::new(db_config).await?);

    // Create integrated transcript service
    let integrated_service = Arc::new(transcript_integration::IntegratedTranscriptService::new(
        Arc::clone(&transcript_service),
        Arc::clone(&s3_service),
        Arc::clone(&db_service),
    ));

    // Start the integrated service
    integrated_service.clone().start().await?;

    // Create transcript API state
    let transcript_api_state = transcript_api::TranscriptApiState {
        transcript_service: Arc::clone(&transcript_service),
        s3_service: Arc::clone(&s3_service),
    ...
    let state: Shared = Arc::new(AppState {
        ...
        transcript_service: Arc::clone(&transcript_service),
        integrated_service: Arc::clone(&integrated_service),
    });

    // Configure CORS for browser WebSocket connections
    ...
    let app = Router::new()
        ...
        .route("/live-transcript/:session_id", get(transcript_websocket::transcript_websocket_handler))
        .route("/transcript/health", get(transcript_websocket::transcript_health_check))
        .with_state(state)
        // REST API routes for transcript management
        .route("/api/transcript/:session_id", axum::routing::get(transcript_api::get_complete_transcript))
        .route("/api/transcript/:session_id/metadata", axum::routing::get(transcript_api::get_session_metadata))
        .route("/api/transcript/:session_id", axum::routing::delete(transcript_api::delete_session))
        .route("/api/user/:user_id/sessions", axum::routing::get(transcript_api::get_user_sessions))
        .with_state(transcript_api_state)
        .layer(cors);

    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await?;
    info!("🚀 Streaming WebSocket on ws://localhost:8080/ws");
    info!("📝 Transcript WebSocket on ws://localhost:8080/live-transcript/{{session_id}}");
    info!("🏥 Transcript health check on http://localhost:8080/transcript/health");
    info!("📊 REST API endpoints:");
    info!("   GET /api/transcript/{{session_id}} - Get complete transcript");
    info!("   GET /api/transcript/{{session_id}}/metadata - Get session metadata");
    info!("   DELETE /api/transcript/{{session_id}} - Delete session");
    info!("   GET /api/user/{{user_id}}/sessions - Get user sessions");
